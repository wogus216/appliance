...
export const samsungAppliances: Appliance[] = [
...

  {
    ...
    errorCodes: [
      ...
      },
      {
        ...
        solution: '2014년 이후 인버터 벽걸이라면 무선 리모컨의 [확인]+[바람세기▽] 또는 [확인]+[무풍]을 5초간 눌러 스마트 리셋을 먼저 해 보세요. 그 이전 모델이거나 리셋이 되지 않으면 분전반 차단기를 내리고 약 1분 뒤 다시 올립니다. 실외기 냉매 배관과 서비스 밸브는 설치 기사 영역이므로 직접 열거나 조이지 마세요. 그래도 반복되면 전동변·냉매 계통 점검이 필요하므로 삼성전자 서비스센터(1588-3366)에 점검을 요청하세요.',
        severity: 'high',
      },
      {
        code: 'E458',
        ...
        solution: '삼성 안내는 "실외기 주변으로 바람을 막는 장애물이 있는지 보고 바람이 잘 통하도록 조치"까지입니다. 실외기실에 설치돼 있다면 갤러리 창을 완전히 여세요 — 닫혀 있으면 보호 제어가 걸립니다. 팬 가드 안으로 손을 넣지 마세요. 전원이 들어와 있으면 보호 제어가 풀리며 갑자기 돌고, 멈춰 있어도 바깥 바람에 돕니다. 앵글로 매달린 실외기라면 접근 자체를 하지 마세요. 그래도 팬이 돌지 않으면 모터 고장이므로 삼성전자 서비스센터(1588-3366)에 점검을 요청하세요.',
        severity: 'high',
      },
      {
        code: 'E464',
        ...
        solution: '실외기 통풍이 막히지 않았는지 확인하고 10분 뒤 재가동하세요. 실외기실에 설치돼 있다면 갤러리 창부터 완전히 여세요 — 삼성이 과열·압력 계열 코드마다 반복해서 안내하는 항목이고, 닫힌 채로는 무엇을 해도 같은 코드가 뜹니다. 반복되면 인버터·컴프레서 문제이므로 삼성전자 서비스센터(1588-3366)에 점검을 요청하세요.',
        severity: 'high',
      },
    ],
  ...
  // === 세탁기 ===
  {
    ...
    errorCodes: [
      ...
      },
      {
        ...
        solution: '필터를 바로 열면 고여 있던 물이 바닥으로 쏟아집니다. 전면 하단 커버를 열고 잔수 제거 호스로 물을 먼저 뺀 뒤 필터를 분리해 청소하세요. 그다음 배수호스 높이와 꼬임을 확인합니다.',
        severity: 'medium',
      },
      {
        code: 'Sud',
  ...
  // === 건조기 ===
  {
    ...
    errorCodes: [
      ...
      },
      {
        ...
        cause: '응축수 물통 가득 참, 배수 호스 꺾임·하수구 막힘, 물통 수납함 분리 후 케이블 연결 불량, 그리고 겨울철 베란다 설치 시 물통·배수 호스 결빙.',
        solution: '물통을 쓴다면 물이 찼는지 먼저 보고 비웁니다. 직배수라면 배수 호스 설치 상태와 하수구 막힘을 확인하세요. 제품을 옮겼거나 물통 수납함을 분리한 적이 있으면 케이블 연결 상태도 봅니다. 겨울에 베란다에 둔 제품이라면 고장이 아니라 결빙인 경우가 많습니다 — 삼성 안내는 배수 호스를 따뜻한 물에 적신 수건으로 10분쯤 감싸 녹이고, 물통 자리에 60℃ 정도의 물을 400ml 이내로 넣어 60분 방치한 뒤 동작시켜 배수를 확인하라고 합니다. 세탁기와 달리 하단 커버 안은 열교환기라 사용자가 분해할 곳이 아닙니다. 그래도 반복되면 삼성전자서비스(1588-3366)에 점검을 요청하세요.',
        severity: 'medium',
      },
      {
        code: 'HC',
    ...
    errorCodes: [
      {
        ...
        solution: '전원을 끄기 전에 냉동식품을 아이스박스나 다른 냉동실로 옮기세요. 문을 연 채 성에가 저절로 녹기를 기다렸다가 재가동합니다. 헤어드라이어·온풍기를 대거나 드라이버·칼로 긁지 마세요 — 내부 라이너가 변형되고 증발기 배관에 구멍이 나면 수리가 아니라 교체가 됩니다. 반복되면 삼성전자 서비스센터(1588-3366)에 문의하세요',
        severity: 'medium',
      },
      {
        code: 'OF F',
    ...
    errorCodes: [
      {
        ...
        solution: '냉동식품을 먼저 옮긴 뒤 전원을 끄고 문을 연 채 성에가 저절로 녹기를 기다렸다가 재가동하세요. 헤어드라이어·온풍기를 대거나 도구로 긁으면 내부 라이너가 변형되고 증발기 배관에 구멍이 나 수리가 아니라 교체가 됩니다. 반복 시 삼성전자 서비스센터(1588-3366)에 문의하세요.',
        severity: 'medium',
      },
      {
        code: '21 E',
        ...
        solution: '반나절 이상 걸리는 작업이므로 냉동식품을 먼저 아이스박스나 다른 냉동실로 옮기세요. 그다음 전원을 끄고 냉동실 문을 연 채 성에를 충분히 녹인 뒤 재가동합니다. 헤어드라이어·온풍기를 대거나 도구로 긁어내지 마세요 — 라이너가 변형되고 증발기 배관이 상합니다. 재가동 후에도 다시 뜨면 팬 모터 점검이 필요하므로 삼성전자 서비스센터(1588-3366)에 문의하세요',
        severity: 'high',
      },
      {
        code: '24 E',
        ...
        solution: '냉동식품을 옮긴 뒤 전원을 끄고 성에를 충분히 녹이면 일시적으로 풀릴 수 있습니다(열기구를 대거나 도구로 긁지 마세요). 다만 제상 부품 자체 문제인 경우가 많아 재발 시 삼성전자 서비스센터(1588-3366) 점검을 권장합니다',
        severity: 'high',
      },
      {
        code: '39 E',
  ...
  // === 세탁기 ===
  {
    ...
    errorCodes: [
      ...
      },
      {
        ...
        solution: '순서가 중요합니다. ① 전원 플러그를 뽑고 ② 전면 하단 커버를 열어 잔수 제거 호스로 물을 먼저 뺀 뒤 ③ 거름망을 분리해 청소합니다. 물이 남은 채 열면 10~20L가 바닥으로 쏟아지고, 삶음·온수 코스 도중이면 뜨겁습니다. 그다음 배수 호스를 펴 주세요. 반복되면 삼성전자 서비스센터(1588-3366)에 문의하세요.',
        severity: 'medium',
      },
      {
        code: 'dC',
    ...
    targetUsers: {
      ...
      notRecommended: [
        ...
        '초기 구입가가 부담스러운 가성비 소비자 (조사 시점 375만원대)',
      ],
    },
  ...
  // === TV (이동식·라이프스타일) ===
  {
    ...
    editorComment:
      'LG 스탠바이미의 강력한 경쟁작으로, 같은 이동식 카테고리에서 120Hz 고주사율과 터치스크린을 앞세웁니다. 게임·스포츠에서 부드러움이 확실히 체감되고, 화면 분리와 풀 모션(틸트·스위블·피벗) 스탠드로 활용도가 높습니다. 다만 100만원대 가격에 QHD 해상도라는 조합은 이 값을 받아들이기 어렵게 만드는 대목이고, 스탠드 포함 25.7kg의 무게와 3시간 배터리(경쟁작보다 1시간 짧음)도 아쉽습니다. 스탠바이미와 나란히 놓으면 갈림길은 분명합니다 — 게임·스포츠에서 120Hz가 필요하면 무빙스타일이고, 배터리 시간과 화면 분리 활용이 우선이면 스탠바이미입니다. 2026년 4월에는 무빙스타일 라인업이 최대 85형까지 확대돼 시리즈 자체가 삼성 TV의 주력 카테고리로 커지고 있습니다.',
    status: 'featured',
  ...
  // === 무선이어폰 ===
  {
    ...
    editorComment:
      '갤럭시 사용자라면 여전히 최우선 후보입니다. 듀얼 드라이버로 해상력이 높고, 적응형 ANC 성능도 동급 상위권이며 실시간 통역·자동 기기 전환 등 삼성 연동 편의가 뛰어납니다. 2026년 3월 후속작 버즈4 프로가 출시되면서 포지션이 바뀌었는데, 해외 기준 정가 대비 30% 이상 할인이 일상화되며 "할인가에 사는 프리미엄"으로 오히려 매력이 커졌습니다. 다만 ANC ON 시 6시간의 짧은 배터리, iOS에서의 기능 제한에 더해, 장기 사용자 사이에서 왼쪽 유닛 충전 불량·이어팁 내구성 같은 품질 이슈 보고가 이어진다는 점은 확인하고 사야 합니다.',
    status: 'best',
    ...
    targetUsers: {
      ...
      notRecommended: [
        ...
        '충전 불량·이어팁 내구성 등 품질 이슈 보고에 민감한 사용자',
      ],
    },
