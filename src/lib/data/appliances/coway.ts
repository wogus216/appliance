...
export const cowayAppliances: Appliance[] = [
  ...
  // === 정수기 ===
  {
    ...
    oneliner: '온수 없는 컴팩트 직수 — 좁은 주방·1인 가구용 정수·냉수',
    editorComment: '코웨이 한뼘 정수기는 저수조를 뺀 컴팩트 직수형으로, 싱크대 옆 자투리 공간이 아쉬운 좁은 주방과 1인 가구를 겨냥합니다. 제품명은 ‘한뼘’이지만 저희가 확인한 사양표의 폭은 240mm라, 자리가 결정적이라면 실측값으로 대조하시기 바랍니다. 저수조 없는 직수 구조에 나노트랩 멤브레인을 더해 위생을 챙겼고, 냉수까지 제공하면서도 온수를 뺀 만큼 체급을 낮춘 모델입니다. 저희는 이 제품의 시중가를 확인하지 못했습니다. 다만 온수가 없어 분유·차를 자주 쓰는 가정엔 상위 냉온정 모델이 맞고, 대가족의 대용량 연속 추출에도 약합니다. 자리·위생·가격의 균형을 원하면 합리적인 선택이지만, 온수가 꼭 필요하면 냉온정으로 올라가야 합니다.',
    status: 'new',
    ...
    roomFit: {
      ...
      coverageArea: 0,
      installationType: '직수형(카운터탑)',
      installationNote: '수도 직결(직수) 설치 — 전문기사 방문 설치 필요. 확인된 사양표 기준 폭 240mm이며, 저수조가 없어 정체수 없이 위생 관리가 간편하다. 방열을 위해 측면·후면에 10cm 이상 통풍 공간 확보 권장.',
    },
