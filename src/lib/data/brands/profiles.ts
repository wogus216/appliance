...
export const brandProfiles: BrandProfile[] = [
  {
    ...
    editorNote:
      '현재 공개 중인 LG 제품은 냉장고·정수기·로봇청소기·TV 4개 카테고리 7개 모델이다. 살림랩이 사양을 정리한 LG 제품은 11개 카테고리 20개 모델로 브랜드 중 가장 넓지만, 모델번호를 공식 자료로 확인하지 못했거나 출처가 한 곳뿐인 나머지는 공개를 보류했다. 공개된 제품에는 오브제컬렉션이 붙은 모델이 다수를 차지해, 인테리어 가전 수요가 구성에 그대로 드러난다.',
    sources: [
      {
  ...
  },
  {
    ...
    editorNote:
      '현재 공개 중인 삼성 제품은 세탁기·건조기·냉장고·무선이어폰·에어컨·로봇청소기·TV 7개 카테고리 9개 모델이다. 살림랩이 사양을 정리한 삼성 제품은 10개 카테고리 17개 모델로 LG 다음으로 폭이 넓지만, 모델번호를 공식 자료로 확인하지 못한 나머지는 공개를 보류했다. 공개된 제품은 비스포크 패널이 붙은 프리미엄 모델이 다수를 차지해 가격대가 전반적으로 높게 형성돼 있다.',
    sources: [
      {
  ...
  },
  {
    ...
    editorNote:
      '현재 공개 중인 애플 제품은 에어팟 프로 3(조사 시점 369,000원) 1종으로, 기본형(199,000원부터)과 맥스(849,000원) 사이 가운데 가격대에 위치한다. H2 칩 기반 노이즈 캔슬링과 심박수 센서·청력 보조를 갖췄지만, 코덱이 SBC·AAC뿐이고 멀티포인트를 정식 지원하지 않아 애플 생태계 밖에서는 값을 다 하지 못한다.',
    sources: [
      {
  ...
  },
  {
    ...
    editorNote:
      '현재 공개 중인 소니 제품은 WF-1000XM5(조사 시점 231,450원) 1종이다. 2026년 2월 후속작 WF-1000XM6이 나오며 세대가 밀렸지만 그만큼 할인폭이 커져 "할인가에 사는 플래그십"으로 가치가 오히려 커졌고, LDAC와 멀티포인트를 동시에 쓸 수 없다는 제약과 IPX4에 그치는 방수 등급은 여전히 남아 있다.',
    sources: [
      {
