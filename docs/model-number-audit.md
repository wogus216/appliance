...
# 2026-09-07 모델번호 재확인 — 제조사 공식 페이지로

/ 측정 도구: `curl` 원본 HTML. **대조군을 먼저 세우고 판정했다**
/ 모집단: `docs/index-starvation-diagnosis.md`가 "공식 확인 대상"으로 지목한 5건 + 제트봇 1건

## 판정 기준(사전 고정) — 대조군부터

앞 회차의 "삼성 지원 페이지 HTTP 500 → 확인 실패"를 그대로 믿지 않고, **실재가 확인된
모델로 같은 요청을 먼저 보냈다.**

| 요청 | 응답 | 뜻 |
| --- | --- | --- |
| `samsung.com/sec/support/model/AR07A9170HCN/` (실재, 전체 SKU) | **200** · 277KB | 정상 |
| `samsung.com/sec/support/model/AR07A9170/` (실재, 접미사 뗌) | **500** · 238KB | — |
| `samsung.com/sec/support/model/ZZ99XX0000Q/` (가짜) | **500** · 238KB | — |
| `lge.co.kr/product/refrigerators/t873mee111` (실재) | **200** · 1.7MB | 정상 |
| `lge.co.kr/product/refrigerators/zz99nonexistent` (가짜) | **404** · 3KB | — |

**결론: 삼성의 500은 "그런 모델 없음"이 아니라 "전체 SKU가 아님"이다.** 실재하는 모델도
접미사를 떼면 똑같이 500을 준다. 따라서 **삼성 쪽 500은 부재의 근거로 쓸 수 없다.**
LG는 실재/부재가 200/404로 깨끗하게 갈려 판정에 쓸 수 있다.

## 판정 결과

| slug | 카탈로그 표기 | 판정 | 근거 |
| --- | --- | --- | --- |
| `lg-codezero-r5-robot` | `R585GA` | **오기 → `RO585HGH`로 정정** | `lge.co.kr/product/vacuum-cleaners/ro585hgh` 200, og:title "LG 코드제로 오브제컬렉션 R5 \| RO585HGH". 다나와 `pcode=77208635` 본품 |
| `lg-trom-obje-fw25eswhs` | `FW25ESWHS` | **정정하지 않음** | 9개 카테고리 경로 전수 404. 근접 모델 `FH25ES`는 200이지만 **워시콤보(세탁·건조 일체형)**라 우리 항목(드럼세탁기)과 **다른 제품**이다 |
| `samsung-bespoke-jetbot-ai` | `VR50T95735W` | **실재하나 미국 SKU** | `samsung.com/us/...VR50T95735W/AA/` 200. 한국 판매 모델은 `VR50T95935W`(`samsung.com/sec` 지원 200) |
| `samsung-inverter-heatpump-dryer-dv10` | `DV10B6320LV` | **불명** | 색상 접미사 3종 모두 500. 대조군상 500은 부재의 근거가 아니다 |
| `samsung-bespoke-dehumidifier-dg16a7500` | `DG16A7500` | **불명(실재 의심)** | 500. 삼성 공식 제습기는 `AY18CG7500GED` 등 `AY` 계열이라 `DG` 접두어가 명명규칙과 어긋난다 |
| `samsung-bespoke-wind-free-af25a9970` | `AF25A9970` | **불명(실재 의심)** | 500. 공식에 존재하는 것은 `AF25M9970`·`AF25N9970` 계열이고 `AF25A9970`은 검색에도 없다 |

## 복구한 것 (1건)

`lg-codezero-r5-robot`을 `UNVERIFIED_SLUGS`에서 빼 공개로 돌렸다. 공개 33 → 34,
**색인가능 16 → 17.**

전수검색(`R585GA`) 7곳 분류 — 변경 3(`name`·`modelNumber`·`image` 경로) · 제거 1
(`unverified.ts`) · **의도적 유지 3**(기록 문서 3곳은 그때의 측정 기록이라 남긴다).
이미지 디렉터리도 `r585ga` → `ro585hgh`로 옮겼다.

## 공개로 돌리자 드러난 것 — 본문의 무근거 수치 4건

404인 동안은 검사를 받지 않던 본문이 공개되면서 회귀 테스트 4개가 한꺼번에 깨졌다.
전부 **출처 없는 수치 주장**이었고, 지웠다.

| 주장 | 처리 |
| --- | --- |
| `소음 66dB` | 삭제. LG 공식이 소음 수치를 공개하지 않는다 |
| `월 예상 전기요금 약 900원` · `월 유지비 4,000원` | 삭제 |
| `정가 149만원 / 실거래 129만원` · `정가 150만원대` | 삭제 |
| `소비전력 60W` | LG 공식값으로 교체 — 본체 터보 30W / 올인원타워 비움 시 1,450W |

**본문 치수도 틀려 있었다.** 본체 `350 x 109 x 350mm` · 타워 `360 x 1080 x 400mm`로 적혀
있었는데 LG 공식은 본체 `342 x 94.5 x 342mm`(3.1kg) · 타워 `202 x 480 x 233mm`(5.4kg)다.
공식값으로 고치고 `VERIFIED_SPECS`에 등재했다.

**이것이 이번 회차의 가장 큰 발견이다** — 비공개 제품의 본문은 회귀 테스트를 통과한 적이
없다. 나머지 40개를 되살릴 때마다 같은 일이 반복된다. 되살리기는 "한 줄 지우기"가 아니다.
