...
**이 API가 하지 않는 일:** 색인 생성 요청. Search Console API에는 요청 엔드포인트가 없고,
구글 Indexing API는 공식적으로 구인공고·라이브방송 전용이라 일반 글에 쓰면 가이드라인 위반이다.
요청 버튼은 여전히 사람이(또는 로그인된 브라우저 자동화로) 누른다. 이 스크립트가 줄여 주는 것은
"어디에 넣을지 고르는 일"과 "무엇을 넣었는지 기록하는 일"이다.

### 2026-09-04 1회차 측정 (사이트맵 77개 전수)

| 상태 | 건수 |
| --- | ---: |
| 색인됨(Submitted and indexed) | 1 |
| **URL is unknown to Google** | **53** |
| Crawled - currently not indexed | 23 |

8/25 GSC 화면 기준(발견됨-미색인 358)과 다른 그림이다. **현재 사이트맵 URL 중 '발견됨-미색인'은
0건이고, 53개가 아예 "구글이 모름"** 이다. 즉 358은 8월 통합 전 옛 URL 쪽 숫자이고, 지금 사이트맵에
올라간 주소 대부분은 구글이 발견조차 못 한 상태다. 사이트맵을 읽고도 큐에 넣지 않았거나 아직 읽지
않았다는 뜻이며, 어느 쪽인지는 **불명**이다.
